# 
# analyzing MIC data from 384-well analysis experiments
# 
...
for(n in 1:length(isolate_labels)) {
  ...
  isolate_rg[[n]] <- rg
  # print(paste('done getting relative growth values for ', rg$isolate, sep = ''))
}
...
# rg_data$label <- paste(rg_data$strain, "_", rg_data$line, "_", rg_data$cycle, sep = "")
rg_data$label <- paste(rg_data$strain, ", line " , substr(rg_data$line, 2, 3), sep = "")
}
...
isolate_names <- unique(rg_data$label)
predicted.drc <- list()
...
for(i in 1:length(isolate_names)) {
  isolate.data <- rg_data %>% filter(label == isolate_names[i])
  drm_fit <- drm(rg~abx_concentration, data = isolate.data, fct = LL.4(fixed = c(NA, NA, NA, NA)))
  
  predicted.drc[[i]] <- data.frame(label = isolate_names[i],
...
ic99s$ic99.label <- paste(ic99s$ic99, ' µg/mL', sep = '')

# creates plot for each line that overlays relative growth points and drm curve fit. also displays IC99 value.
pdf(file = paste('../figures/mic-', date,  '/rg+ic99_', cycle, '.pdf', sep = ''), height = 6, width = 12)
ggplot(data = rg_data, aes(x = as.numeric(abx_concentration), y = rg, color = label)) +
  geom_point(alpha = 0.5) +
  geom_line(data = predicted_rg, aes(x = as.numeric(abx_concentration), y = pred.growth)) +
  geom_text(data = ic99s, mapping = aes(x = 1, y = 5, label = ic99.label, group = label)) +
  ...
  ggtitle(substitute(paste('Relative Growth and MIC of WT and ', italic('fas Msm'), ' Strains Treated with Isoniazid'))) + xlab('isoniazid (µg/mL)') + ylab('relative growth') +
  plot_theme +
